...
model CylinderGroundMeshInnerSlope "(Hollow) cylinder ground mesh with inclined inner surface"
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a slopePort[nElementsR] "inner port" annotation(
  ...
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_b outerPort[nElementsR] "Thermal port for 1-dim. heat transfer (unfilled rectangular icon)" annotation(
    Placement(transformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 180), iconTransformation(origin = {100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  parameter Integer nElementsR;
  parameter Modelica.Units.SI.Length meshR[nElementsR + 1];
  parameter Modelica.Units.SI.Length meshZ[nElementsR + 1];
  parameter Modelica.Units.SI.Temperature Tinitial[nElementsR] "Depth dependent initial temperature";
  replaceable parameter Parameters.Soils.Soil groundData constrainedby MoSDH.Parameters.Soils.SoilPartial annotation(
     choicesAllMatching = true);
  GroundElement groundElements[nElements](each groundData = groundData, innerRadius(each fixed = true) = {if elementCoordinates[iEl, 1] <> elementCoordinates[iEl, 2] then meshR[elementCoordinates[iEl, 2]] else meshR[elementCoordinates[iEl, 2] + 1] - (meshR[elementCoordinates[iEl, 2] + 1] - meshR[elementCoordinates[iEl, 2]]) / sqrt(2) for iEl in 1:nElements}, outerRadius(each fixed = true) = {meshR[elementCoordinates[iEl, 2] + 1] for iEl in 1:nElements}, elementHeight(each fixed = true) = {if elementCoordinates[iEl, 1] <> elementCoordinates[iEl, 2] then meshZ[elementCoordinates[iEl, 1] + 1] - meshZ[elementCoordinates[iEl, 1]] else (meshZ[elementCoordinates[iEl, 1] + 1] - meshZ[elementCoordinates[iEl, 1]]) / sqrt(2) for iEl in 1:nElements}, Tinitial = {Tinitial[elementCoordinates[iEl, 1]] for iEl in 1:nElements}) "Vector of ground elements" annotation(
    Placement(transformation(extent = {{-10, -10}, {10, 10}})));
protected
  parameter Integer nElements = sum(r for r in 1:nElementsR) "Number of elements in the triangular region";
  parameter Integer elementCoordinates[nElements, 2] = MoSDH.Components.Storage.PitStorage.Functions.slopeElementCoordinates(isInnerSlope = true, nElements = nElements, nElementsR = nElementsR);
  parameter Integer elementIndices[nElementsR, nElementsR] = MoSDH.Components.Storage.PitStorage.Functions.slopeElementIndice(isInnerSlope = true, nElementsR = nElementsR);
initial algorithm
...
/*------ EMBANKMENT ELEMENT NUMBERING 'i' -----
  ...
  for z in 1:nElementsR loop
    for r in 1:nElementsR loop
      if elementIndices[z, r] > 0 then
//element found
// bottom port connection
        if z == nElementsR then
          connect(groundElements[elementIndices[z, r]].bottomHeatPort, bottomPort[r]) annotation(
            Line(points = 0));
        else
          connect(groundElements[elementIndices[z, r]].bottomHeatPort, groundElements[elementIndices[z + 1, r]].topHeatPort);
        end if;
// outer port connection (to right element)
        if r == nElementsR then
          connect(groundElements[elementIndices[z, r]].outerHeatPort, outerPort[z]);
        else
          connect(groundElements[elementIndices[z, r]].outerHeatPort, groundElements[elementIndices[z, r + 1]].innerHeatPort);
        end if;
// connect inner ports
        if r == nElementsR - z + 1 then
          connect(groundElements[elementIndices[z, r]].innerHeatPort, slopePort[z]);
          connect(groundElements[elementIndices[z, r]].topHeatPort, slopePort[z]);
        end if;
      end if;
    end for;
  end for;
  annotation(
    Icon(graphics = {Polygon(points = {{-103.7, -100}, {99.59999999999999, -100}, {99.59999999999999, 100}}, fillColor = {210, 180, 140}, fillPattern = FillPattern.Solid), Bitmap(imageSource = "iVBORw0KGgoAAAANSUhEUgAAATwAAAEPCAYAAADf63gFAAAABGdBTUEAALGPC/xhBQAAAAlwSFlz
...
Kbsy5I8PuuDS8I1i6ExKKbu2mUe4ElIdSSllly6Bpz3t/wEy440VbTnrngAAAABJRU5ErkJggg==", extent = {{36.2, -102.9}, {106.7, -46.7}}), Bitmap(imageSource = "
...
Dzu/Kzt34EEIPRoKoUfjBB8dMeNDOpdylq80/85MhVpKtr3RTLUlx2fImxB05BCZFMIuzOw2FHYQ
XhxmeiH4fPgF6VzKNl5hcnLy35rc9YQbAeC3TU5nud98sZl6rRyvIWtCyPmgC2G3MCH0msHnw09K
Kbsy5I8PuuDS8I1i6ExKKbu2mUe4ElIdSSllly6Bpz3t/wEy440VbTnrngAAAABJRU5ErkJggg==", extent = {{-23.6, -103.1}, {46.9, -46.9}}), Bitmap(imageSource = "
...
Dzu/Kzt34EEIPRoKoUfjBB8dMeNDOpdylq80/85MhVpKtr3RTLUlx2fImxB05BCZFMIuzOw2FHYQ
XhxmeiH4fPgF6VzKNl5hcnLy35rc9YQbAeC3TU5nud98sZl6rRyvIWtCyPmgC2G3MCH0msHnw09K
Kbsy5I8PuuDS8I1i6ExKKbu2mUe4ElIdSSllly6Bpz3t/wEy440VbTnrngAAAABJRU5ErkJggg==", extent = {{-82.90000000000001, -102.8}, {-12.4, -46.6}}), Bitmap(imageSource = "
...
Dzu/Kzt34EEIPRoKoUfjBB8dMeNDOpdylq80/85MhVpKtr3RTLUlx2fImxB05BCZFMIuzOw2FHYQ
XhxmeiH4fPgF6VzKNl5hcnLy35rc9YQbAeC3TU5nud98sZl6rRyvIWtCyPmgC2G3MCH0msHnw09K
Kbsy5I8PuuDS8I1i6ExKKbu2mUe4ElIdSSllly6Bpz3t/wEy440VbTnrngAAAABJRU5ErkJggg==", extent = {{35.8, -52.5}, {106.3, 3.7}}), Bitmap(imageSource = "
...
Dzu/Kzt34EEIPRoKoUfjBB8dMeNDOpdylq80/85MhVpKtr3RTLUlx2fImxB05BCZFMIuzOw2FHYQ
XhxmeiH4fPgF6VzKNl5hcnLy35rc9YQbAeC3TU5nud98sZl6rRyvIWtCyPmgC2G3MCH0msHnw09K
Kbsy5I8PuuDS8I1i6ExKKbu2mUe4ElIdSSllly6Bpz3t/wEy440VbTnrngAAAABJRU5ErkJggg==", extent = {{-23.5, -52.2}, {47, 4}}), Bitmap(imageSource = "
...
Dzu/Kzt34EEIPRoKoUfjBB8dMeNDOpdylq80/85MhVpKtr3RTLUlx2fImxB05BCZFMIuzOw2FHYQ
XhxmeiH4fPgF6VzKNl5hcnLy35rc9YQbAeC3TU5nud98sZl6rRyvIWtCyPmgC2G3MCH0msHnw09K
Kbsy5I8PuuDS8I1i6ExKKbu2mUe4ElIdSSllly6Bpz3t/wEy440VbTnrngAAAABJRU5ErkJggg==", extent = {{36.2, -0.9}, {106.7, 55.3}})}),
    experiment(StopTime = 1, StartTime = 0, Tolerance = 1e-06, Interval = 0.001));
end CylinderGroundMeshInnerSlope;
